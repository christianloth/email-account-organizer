    ...
    getJSON('emails.json',
            function(err, data){
                ...
                div.setAttribute('class', 'd-flex flex-row justify-content-between align-items-center Email Section');
                    ...
                    getJSON("checkAccount("+ accountJSON +").json", function(er, reputation){
                        var rep = JSON.parse(reputation);
                        if(rep.suspicious){
...
function reccomendJoke(category){   // Recommends joke based on user input.
    ...
    getJSON("joke("+ category +").json", function(err, jk){

        if(err != 200){
            window.alert("Error: " + err + "\nCould not retrieve joke.");
        }
        else{
            var joke = JSON.parse(jk);
            ...
            */
            ...
            window.alert("Joke had been added to end of draft.");
        }
    });
}
